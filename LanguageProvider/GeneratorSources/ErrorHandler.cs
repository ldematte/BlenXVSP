...
namespace Babel.Parser
{
    ...
    public class ErrorHandler : IErrorHandler
    {
        ...
        public ErrorHandler(IServiceProvider service)
        {
           errors = new List<Error>(8);
           errorProvider = new ErrorListProvider(service);
        }
        // -----------------------------------------------------
        ...
        public void AddError(string msg, int lin, int col, int len, int severity)
        {
           ...
           if (fileName != null)
           {
              newError.Document = fileName;
              newError.Navigate += new EventHandler(delegate(object sender, EventArgs e)
              {
                 errorProvider.Navigate(newError, VSConstants.LOGVIEWID_TextView);
              });
           }
